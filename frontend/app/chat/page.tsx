      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 flex flex-col md:flex-row gap-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
              <Checkbox 
                ...
                id="all-sources"
              />
              <label htmlFor="all-sources" className="text-sm font-medium">All Sources</label>
            </div>
            {/* Individual Sources */}
            {SOURCES.map((src) => (
              <div key={src.value} className="flex items-center space-x-2">
                <Checkbox
                  ...
                  id={src.value}
                />
                <label htmlFor={src.value} className="text-sm">{src.label}</label>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="md:hidden mb-4">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="w-full">Select Sources</Button>
            </DropdownMenuTrigger>
        ...
        <div className="flex-1">
          ...
          <div className="space-y-4 max-h-[700px] overflow-y-auto border rounded-lg p-5 bg-card text-lg leading-relaxed scrollbar-none">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.type === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`rounded-lg p-3 max-w-[85%] ${msg.type === "user" ? "bg-primary text-white" : "bg-background text-foreground"}`}>
                  {msg.type === "user" ? (
                    <p>{msg.content}</p>
                  ) : (
                    <>
                      ...
                      {msg.content.references?.map((ref: any, j: number) => (
                        <div key={j} className="p-3 mt-2 rounded-lg border bg-card space-y-1">
                          {ref.type === "Quran" ? (
                            <>
                              <p className="font-semibold">📖 {ref.reference} ({ref.surah} {ref.ayahNumber})</p>
          ...
          <form onSubmit={handleSearch} className="mt-4 flex flex-col sm:flex-row items-center gap-2">
            ...
            />
            <Button type="submit" disabled={isSearching || !searchQuery.trim()} className="h-12 w-full sm:w-12 p-0">
